...
static inline void perturb(int g, int i, double *c)
/* 
 * g: group index
 * i: individual index
 ...
 * */
{
  double xmax;
#if Accumulatedpayoff
  xmax = pow( ( (1-Discount)*Api[g][i] + 1.0 + B*V[g][i])/C, I_Alpha);       // upper bound of x at each role   
#else
  xmax = pow( (1.0 + B*V[g][i])/C, I_Alpha);                                 // upper bound of x at each role 
#endif
  c[0] = normal(x[g][i], Sigma);                                             // perturbation in effort
  if( c[0] > xmax) c[0] = xmax;
  else if( c[0] < 0.0) c[0] = 0.0;
  
#if PUNISH
  c[1] = normal(dxi[g][i], Sigma_dxi);                                       // perturb threshold 
  c[1] = MAX(c[1],0);
#if AGGR
  c[2] = normal(dsi[g][i], Sigma_dsi);                                       // perturbation in aggressiveness     
  c[2] = MAX(c[2], 0.0);  
#endif
#endif  
}
...
static inline void perturb_1(int g, int i, double *c, double xx, double t, double a, int m)
/* 
 * g: group index
 * i: individual index
 * c: candidate strategy vector pointer 
...
#if FORESIGHT
double foresight(int g, int i, int ng, double xb, double gx, double *cs)
{
  int h, j, c, m;  
  static double tgx, tp, bvg, f, pgx, wx, api;
  ...
  for( wx = 0, j = 0; j < GS[g]; j++){
	...
	if(h){ // perturbation
	  /*FCS[h][0] = normal(Strat[j][0], Sigma);                                   // perturb effort
	  if(FCS[h][0] > Xmax[j]){
	    FCS[h][0] = Xmax[j];
	  }
	  else if(FCS[h][0] < 0.0){
	  ...
	  FCS[h][1] = normal(Strat[j][1], Sigma_dxi);                               // perturb threshold
	  FCS[h][1] = MAX(FCS[h][1], 0.0);
	  #if AGGR
	    FCS[h][2] = normal(Strat[j][2], Sigma_dsi);                             // perturb aggressiveness
	    FCS[h][2] = MAX(FCS[h][2], 0.0);
	  #endif    
	  #if AGGR == 0
	    if(!m) FCS[h][1] = Strat[j][1];                                        // if m !=1, it means j does not punish and use old threshold
	  #endif  */
	  perturb_1(g, j, FCS[h], Strat[j][0], Strat[j][1], Strat[j][2], m);
	}
	tgx = pgx + pow(FCS[h][0], I_Gamma);                                     // add effort of individual which is generating foresight strategy
	tgx = pow(tgx, GaBe);                                                    // new group effort with beta power
	f = get_payoff(g, j, tgx, xb, bvg, api, FCS[h], Lt, Idx, Strat); 
	/*f   = F0*(1. + bvg*tgx/(tgx+xb) - ( C*pow(FCS[h][0], Alpha) ) );         // payoff after production of new strategy for j  
	#if Accumulatedpayoff
	  f   = (1-Discount)*Api[g][j] + f;    
	#endif
	f   = MAX(f, MINPAYOFF);
	f   = expectedPayoffAfterPunishment(g, j, FCS[h], f, Lt, Idx, Strat);      // deduction of expected punishment        
	f   = MAX(f, MINPAYOFF); 
	*/
  ...
  wx = pow(wx, GaBe);
  /*f = F0*(1. + B*ng*V[g][i]*wx/(wx+xb) - ( C*pow(FS[i][0], Alpha) ) );            // payoff after production of new strategy for i using foresighted strategies fs
  #if Accumulatedpayoff
    f   = (1-Discount)*Api[g][i] + f;    
  #endif
  f   = MAX(f, MINPAYOFF);
  f = expectedPayoffAfterPunishment(g, i, cs, f, Lookup, Lidx, FS);            // payoff expected after foresight
  f = MAX(f, MINPAYOFF);
  */
  f = get_payoff(g, i, wx, xb, B*ng*V[g][i], (1-Discount)*Api[g][i], FS[i], Lookup, Lidx, FS);
  return f;
}
#endif
...
void updateStrategy(int g, int i, int ng, double wgx)
/*
 * g: group index
 * i: individual index
 * ng: no. of groups involved in a game
 * wgx: sum of groups contribution except 'g' group with Beta exponent playing the game
 */
{
  int j, c, m;
  double gx, tgx, f, tp, bvg, api = 0;    
  ...
  for(tp = -1, c = 0, j = 0; j < K; j++){                                 // calculate expected payoff for next K candidate strategies and use exponent of it as probability of getting selected
    if(!j){
      ...
      //f = F0*(1.0 + bvg*P[g] - (C*pow(CS[0][0], Alpha)) );               // expected payoff without punishment for current strategy    
      //tgx = gx + pow(CS[j][0], I_Gamma);
      //tgx = pow(tgx, GaBe);  
    }
    else{
      /*perturb(g, i, CS[j]);          
      #if AGGR == 0
	if(!m) CS[j][1] = dxi[g][i];                                         // if m !=1, it means i does not punish and use old threshold
      #endif
      */
      perturb_1(g, i, CS[j], x[g][i], dxi[g][i], dsi[g][i], m);              // perturb strategies     
      
    }
    tgx = gx + pow(CS[j][0], I_Gamma);                                   // group effort with new candidate strategy effort
    tgx = pow(tgx, GaBe);                                                // new group effort with beta power
    /*f   = F0*(1. + bvg*tgx/(tgx+wgx) - ( C*pow(CS[j][0], Alpha) ) );     // payoff after production of new strategy for j  
    #if Accumulatedpayoff
      f   = api + f;    
    #endif
    f   = MAX(f, MINPAYOFF);      
    */
    f = get_payoff(g, i, tgx, wgx, bvg, api, CS[j], Lookup, Lidx, Tx);     // level 1 payoff
    #if PUNISH      
      //f = expectedPayoffAfterPunishment(g, i, CS[j], f, Lookup, Lidx, Tx);       // level 1 payoff
      #if FORESIGHT
	// copy strategies for i
	Strat[i][0] = CS[j][0];                                                          // copy effort
	Strat[i][1] = CS[j][1];                                                          // copy threshold
	Strat[i][2] = CS[j][2];                                                          // copy aggressiveness  
	f2 = foresight(g, i, ng, wgx, tgx, CS[j]);    // level 2 payoff 
